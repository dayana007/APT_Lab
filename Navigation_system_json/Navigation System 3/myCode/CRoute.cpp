...
bool CRoute::connectToWpDatabase(CWpDatabase* pWpDB)
{
	if(pWpDB== NULL)
	{
		cout<<" Error : No Database in the project "<<endl;
...
bool CRoute::addPoi(string namePoi,string afterWp)
{
	...
	//cout<<"Error: No POI database found";

	if((m_pPoiDatabase == NULL)||(m_pWpDatabase == NULL))
	{
		cout<<"Error: No POI or Waypoint database found"<<endl;
		return false;
	}
	else
	{
...
bool CRoute::addWaypoint(string name)
{
	CWaypoint* pWp;
...
{
	vector<const CWaypoint*> vecRoute;
	CWaypoint* pWp;
	list<CWaypoint*>::iterator Itr;
	Itr =listRoute.begin();
	while(Itr != listRoute.end())
	{
		...
		vecRoute.push_back(pWp);
		Itr++;
	}
	return static_cast< const vector<const CWaypoint*> >(vecRoute);

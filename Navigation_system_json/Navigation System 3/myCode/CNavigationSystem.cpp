...
{
	/**
	 * Databse can be filled by reading from csv file.
	 * But csv file can be written from databse entries too
	 * so the below code is commented and when needed to be wriiten
	 * to csv file from database, this can be uncommented
	 */
...
//}
/**
 * Read from persistence storage to database
 * return true if read is succesful
 */

bool CNavigationSystem::readFromPersistentStorage(CCSVStorage::MergeMode mode)
{
	bool bResult = false;
	//bResult = m_CPerObj.readData(m_WpDatabase,m_PoiDatabase,mode);
	m_CPerJsonObj.readData(m_PoiDatabase,m_WpDatabase);
	//bResult = m_CPerObj.readData(m_WpDatabase,m_PoiDatabase,CCSVStorage::MERGE);
	return bResult;
}

/**
 * Write to persistence storage from database
...
bool CNavigationSystem::writeToPersistentStorage()
{
	...
	m_CPerJsonObj.writeData(m_WpDatabase,m_PoiDatabase);
	return bResult;
}

/**
 * set media name of the persistence storage used
 * return void
 */
void CNavigationSystem::setMedia(string name)
{
	m_CPerObj.setMediaName(name);
	m_CPerJsonObj.setMediaName(name);
}

...
void CWaypoint::transformLatitude2degmmss(int& deg, int& mm, double& ss)
{
	...
	mintosec = degtomin*60.0;
	ss =(mintosec - mm)*60.0;
	cout<<this->getName()<<" on latitude = "<<deg<<"deg "<<mm<<"min "<<ss<<"sec"<<endl;
}
/**
 * Default destructor
 */
 ...
 * @return void
 */
void CWaypoint::setType(wp_type type)
{
	m_type = type;
}
/**
 ...
 */
wp_type CWaypoint::getType() const
{
	wp_type type;
	type = m_type;
	return type;
 ...
 */
ostream& operator <<(ostream& out, CWaypoint& origin)
{
	origin.print(MMSS);
	return out;
